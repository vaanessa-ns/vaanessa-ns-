...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // 1. Auth check route (/api/pluggy/auth)
  if (view === 'auth' || url.includes('/auth')) {
    try {
      ...
      console.warn('[API /api/pluggy/diagnostics?view=auth] Erro na autenticação:', error?.message || error);
  ...
  // 2. Logs route (/api/pluggy/logs)
  if (view === 'logs' || url.includes('/logs') || req.query?.format || req.query?.errorsOnly) {
    ...
    try {
      if (action === 'test' || req.method === 'POST') {
        ...
        console.log('[API /api/pluggy/diagnostics] Live diagnostic probe:', probeResult.success ? 'SUCCESS' : 'FAILED');
      }
      ...
      console.warn('[API /api/pluggy/diagnostics] Erro ao recuperar logs de diagnóstico:', error?.message || error);
      return res.status(500).json({
        success: false,
  ...
  // 3. Default Diagnostics route (/api/pluggy/diagnostics)
  try {
    const diag = await getPluggyDiagnostics();
    return res.status(200).json(diag);
  } catch (error: any) {
    console.warn('[API /api/pluggy/diagnostics] Erro nos diagnósticos:', error?.message || error);
    return res.status(500).json({
